                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                  ...
                  <div className="space-y-4">
                    ...
                    {workflowSteps.length > 0 ? (() => {
                      // Find if there's a rejection in the workflow
                      const rejectedStepIndex = workflowSteps.findIndex((step: any) => step.status === 'Rejected');
                      const isRejected = rejectedStepIndex !== -1;
                      
                      return workflowSteps.map((step: any, index: number) => {
                        // Determine if this step should show "Not Required" (steps after rejection)
                        const isAfterRejection = isRejected && index > rejectedStepIndex;
                        const stepStatus = isAfterRejection ? 'Not Required' : step.status;
                        ...
                        return (
                          <div key={index} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                              ...
                              stepStatus === 'Not Required' ? 'bg-gray-100' : 'bg-gray-200'
                            }`}>
                              {stepStatus === 'Approved' ? (
                                <CheckCircle className="w-5 h-5 text-green-600" />
                              ) : stepStatus === 'Rejected' ? (
                                <AlertCircle className="w-5 h-5 text-red-600" />
                              ) : stepStatus === 'Not Required' ? (
                                <span className="w-5 h-5 text-gray-400 text-xs font-bold">N/A</span>
                              ) : (
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-xs text-gray-500">
                                {stepStatus === 'Approved' ? (
                                  <>
                                    Approved {step.approvedAt ? format(new Date(step.approvedAt), 'MMM dd, yyyy HH:mm') : ''}
                                    {step.displayActualApprover && (
                                      <span className="ml-2 text-blue-600">by {step.displayActualApprover}</span>
                                    )}
                                  </>
                                ) : stepStatus === 'Rejected' ? (
                                  <>
                                    Rejected {step.approvedAt ? format(new Date(step.approvedAt), 'MMM dd, yyyy HH:mm') : ''}
                                    {step.displayActualApprover && (
                                      <span className="ml-2 text-red-600">by {step.displayActualApprover}</span>
                                    )}
                                    {step.rejectionReason && (
                                      <div className="mt-1 text-red-600 font-medium">Reason: {step.rejectionReason}</div>
                                    )}
                                  </>
                                ) : stepStatus === 'Not Required' ? (
                                  'Not required due to rejection'
                                ) : (
                                  'Pending approval'
                                )}
                              </div>
                    ...
                    {/* Finance Payment Step - Show as "Not Required" if trip is rejected */}
                    {(() => {
                      const isRejected = workflowSteps.some((step: any) => step.status === 'Rejected');
                      const isPaid = detailedTripData?.paid || selectedRequest.paid;
                        ...
                        <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500">
                              {isRejected ? 
                                'Not required due to rejection' :
                                isPaid && (detailedTripData?.paidAt || selectedRequest.paidAt) ? 
