...
export default function AdminPage() {
  ...
  // Fetch users for display (basic info)
  const { data: users = [], isLoading: isLoadingUsers } = useQuery({
    queryKey: ["/api/users"],
  });

  // Fetch basic user info for non-admin operations
  ...
  // Handle opening edit department dialog
  ...
  // Handle opening user role change dialog
  ...
  const handleChangeUserRole = (data: z.infer<typeof roleChangeSchema>) => {
    if (selectedUser) {
  ...
  return (
    <Layout>
      ...
      <Dialog open={userRoleDialogOpen} onOpenChange={setUserRoleDialogOpen}>
        <ResizableDialogContent className="w-[400px] max-w-[90vw]">
          <DialogHeader>
            <DialogTitle>Change User Role</DialogTitle>
            <DialogDescription>
              Update the role for {selectedUser?.fullName || "this user"}.
            </DialogDescription>
          </DialogHeader>
          
          <Form {...roleChangeForm}>
            <form onSubmit={roleChangeForm.handleSubmit(handleChangeUserRole)} className="space-y-4">
              <FormField
