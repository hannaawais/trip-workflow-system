...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (systemSettings && Array.isArray(systemSettings) && systemSettings.length > 0) {
      ...
      const homeDeductionKmSetting = (systemSettings as any[]).find((setting: any) => setting.settingKey === 'home_deduction_km');
      const homeDeductionMinSetting = (systemSettings as any[]).find((setting: any) => setting.settingKey === 'home_deduction_min_distance');
      const homeDeductionEnabledSetting = (systemSettings as any[]).find((setting: any) => setting.settingKey === 'home_deduction_enabled');
      
      if (maxKmSetting) {
        systemSettingsForm.setValue('maxKilometers', maxKmSetting.settingValue);
      }
      if (homeDeductionKmSetting) {
        systemSettingsForm.setValue('homeDeductionKm', homeDeductionKmSetting.settingValue);
      }
      if (homeDeductionMinSetting) {
        systemSettingsForm.setValue('homeDeductionMinDistance', homeDeductionMinSetting.settingValue);
      }
      if (homeDeductionEnabledSetting) {
        systemSettingsForm.setValue('homeDeductionEnabled', homeDeductionEnabledSetting.settingValue === 'true');
      }
    }
  }, [systemSettings]);
  ...
  const systemSettingSchema = z.object({
    ...
    homeDeductionKm: z.string().refine(val => !isNaN(Number(val)) && Number(val) >= 0, {
      message: "Value must be a positive number",
    }),
    homeDeductionMinDistance: z.string().refine(val => !isNaN(Number(val)) && Number(val) >= 0, {
      message: "Value must be a positive number",
    }),
    homeDeductionEnabled: z.boolean(),
  });
  ...
  const systemSettingsForm = useForm<z.infer<typeof systemSettingSchema>>({
    ...
    defaultValues: {
      maxKilometers: "0",
      homeDeductionKm: "25",
      homeDeductionMinDistance: "30",
      homeDeductionEnabled: true,
    }
  });
  ...
  const departmentForm = useForm<z.infer<typeof departmentSchema>>({
    ...
    defaultValues: {
      ...
      secondManagerId: null as any,
      thirdManagerId: null as any,
  ...
  const editUserMutation = useMutation({
    mutationFn: async (data: { id: number; fullName: string; email: string; department: string; companyNumber: string; homeAddress: string; homeLocation?: string; directManagerName?: string; directCostEntryPermission: boolean; role?: string }) => {
  ...
  const updateProjectMutation = useMutation({
    mutationFn: async (data: { id: number; name: string; budget: number; departmentId?: number; managerId?: number; secondManagerId?: number | null; expiryDate?: Date; isActive: boolean }) => {
  ...
  const filteredSites = Array.isArray(sites) ? sites.filter((site: any) => {
  ...
  const handleUpdateSettings = async (data: z.infer<typeof systemSettingSchema>) => {
    setIsSavingSettings(true);
    
    // Update all system settings
    const settingsToUpdate = [
      { key: 'maxKilometers', value: data.maxKilometers },
      { key: 'home_deduction_km', value: data.homeDeductionKm },
      { key: 'home_deduction_min_distance', value: data.homeDeductionMinDistance },
      { key: 'home_deduction_enabled', value: data.homeDeductionEnabled.toString() },
    ];
    ...
    try {
      ...
      for (const setting of settingsToUpdate) {
        await new Promise((resolve, reject) => {
          updateSystemSettingMutation.mutate(setting, {
            onSuccess: resolve,
            onError: reject,
          });
        });
      ...
      toast({
        ...
        description: "Some settings may not have been updated properly.",
        variant: "destructive",
      });
    } finally {
      setIsSavingSettings(false);
  ...
  const handleCreateDepartment = (data: z.infer<typeof departmentSchema>) => {
    ...
    const formData = {
      ...
      // Keep null values to properly clear manager fields in database
      managerId: data.managerId,
      secondManagerId: data.secondManagerId,
      thirdManagerId: data.thirdManagerId,
  ...
  const handleCreateProject = (data: z.infer<typeof projectSchema>) => {
    ...
    const formData = {
      ...
      // Ensure required fields are properly set (not null)
      departmentId: data.departmentId as number,
      managerId: data.managerId as number,
      // Keep null values for optional secondary manager
      secondManagerId: data.secondManagerId,
    };
    ...
    if (isEditing && selectedProject) {
      ...
      // Convert null to undefined for createProject API compatibility
      const createData = {
        ...formData,
        departmentId: formData.departmentId as number,
        managerId: formData.managerId as number,
        secondManagerId: formData.secondManagerId || undefined,
      };
      createProjectMutation.mutate(createData);
    }
  };
    ...
    <Layout>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {filteredDepartments.map((dept: any) => {
                            ...
                            const manager = Array.isArray(users) ? users.find((user: any) => user.id === dept.managerId) : undefined;
                            // Find parent department
                            const parentDept = Array.isArray(departments) ? departments.find((d: any) => d.id === dept.parentDepartmentId) : undefined;
                        ...
                        <TableBody>
                          {filteredProjects.map((proj: any) => (
                            <TableRow key={proj.id}>
                              ...
                              <TableCell>
                                {Array.isArray(departments) ? departments.find((dept: any) => dept.id === proj.departmentId)?.name || 'Not assigned' : 'Not assigned'}
                              </TableCell>
                    ...
                    {isLoadingSettings ? (
                      ...
                      <Form {...systemSettingsForm}>
                        <form onSubmit={systemSettingsForm.handleSubmit(handleUpdateSettings)} className="space-y-8">
                          ...
                          {/* Home Trip Deduction Settings */}
                          <div className="border-t pt-6">
                            <h3 className="text-lg font-semibold mb-4">Home Trip Deduction Settings</h3>
                            
                            <FormField
                              control={systemSettingsForm.control}
                              name="homeDeductionEnabled"
                              render={({ field }) => (
                                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                                  <div className="space-y-0.5">
                                    <FormLabel>Enable Home Trip Deduction</FormLabel>
                                    <FormDescription>
                                      When enabled, trips from/to employee home locations will have kilometers deducted based on company policy.
                                    </FormDescription>
                                  </div>
                                  <FormControl>
                                    <Switch
                            ...
                            <FormField
                              control={systemSettingsForm.control}
                              name="homeDeductionKm"
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Home Trip Deduction Amount (km)</FormLabel>
                                  <FormControl>
                                    <Input type="number" min="0" step="1" {...field} />
                                  </FormControl>
                                  <FormDescription>
                                    Number of kilometers to deduct from home trips as per company policy.
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
                              )}
                            ...
                            <FormField
                              control={systemSettingsForm.control}
                              name="homeDeductionMinDistance"
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Minimum Distance for Deduction (km)</FormLabel>
                                  <FormControl>
                                    <Input type="number" min="0" step="1" {...field} />
                                  </FormControl>
                                  <FormDescription>
                                    Minimum total trip distance required to apply the home deduction. Trips shorter than this will not have deductions applied.
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
                              )}
              ...
              {activeTab === "km-rates" && (
                <Card>
                  ...
                  <CardContent>
                    {isLoadingKmRates ? (
                    ...
                    ) : Array.isArray(kmRates) && kmRates.length === 0 ? (
                      <div className="text-center py-8">
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {Array.isArray(kmRates) && kmRates.map((rate: any) => {
                              ...
                              <TableCell>
                                <span className={`px-2 py-1 rounded-full text-xs ${
                                  site.siteType === "Hospital" 
                                    ? 'bg-red-100 text-red-800'
                                    : site.siteType === "Comprehensive clinic"
                                    ? 'bg-blue-100 text-blue-800'
                      ...
                      <FormControl>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            {(users as any[])?.filter((user: any) => ["Manager", "Admin"].includes(user.role))
                              .map((user: any) => (
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="none">None</SelectItem>
                          {(departments as any[])?.filter((dept: any) => dept.isActive && (!selectedDepartment || dept.id !== selectedDepartment.id)).map((dept: any) => (
                    ...
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          {(departments as any[])?.filter((dept: any) => dept.isActive).map((dept: any) => (
                            <SelectItem key={dept.id} value={dept.id.toString()}>
                              {dept.name}
                            </SelectItem>
                          ))}
                    ...
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          {(users as any[])?.filter((user: any) => ["Manager", "Admin"].includes(user.role)).map((user: any) => (
                            <SelectItem key={user.id} value={user.id.toString()}>
                              {user.fullName}
                            </SelectItem>
                          ))}
      ...
      <Dialog open={projectDetailDialogOpen} onOpenChange={setProjectDetailDialogOpen}>
        <ResizableDialogContent className="w-[800px] max-w-[90vw]">
          ...
          {selectedProject && (
            <ProjectDocuments projectId={selectedProject.id} isProjectManager={true} />
          )}
        </ResizableDialogContent>
      </Dialog>
