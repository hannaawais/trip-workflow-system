...
export default function ApprovalsPage() {
  ...
  const approvalMutation = useMutation({
    ...
    onError: (error: Error) => {
      ...
      if (error.message.includes("budget is exceeded") || error.message.includes("Budget exceeded") || error.message.includes("Project budget exceeded")) {
        title = "Budget Exceeded";
        description = error.message.replace("This trip cannot be approved because the ", "The ");
      } else if (error.message.includes("permission")) {
        title = "Permission Denied";
      } else if (error.message.includes("not found")) {
        title = "Request Not Found";
      } else {
        // Show a friendlier generic message with budget context
        description = "This approval cannot be processed. This may be due to budget constraints or other business rules. Please check the project budget or contact Finance for assistance.";
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                console.log("[Refresh Button] Invalidating approval data caches...");
                queryClient.invalidateQueries({ queryKey: ["/api/trip-requests/pending-approval"] });
                queryClient.invalidateQueries({ queryKey: ["/api/admin-requests"] });
                toast({
                  title: "Data refreshed",
                  description: "Approval data has been updated with latest information",
                });
              }}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-neutral-700">{selectedRequest.origin || 'Not specified'}</p>
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-neutral-700">{selectedRequest.ticketNo || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-neutral-500">KM Rate Applied:</p>
                      <p className="text-sm text-neutral-700">{selectedRequest.kmRateValue ? `${selectedRequest.kmRateValue} JD/km` : '0.15 JD/km'}</p>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm font-medium text-neutral-500">Home Trip Deduction:</p>
                      <p className="text-sm text-neutral-700">
                        {selectedRequest.homeDeductionKm 
                          ? `${selectedRequest.homeDeductionKm} km deducted`
                          : 'Not applied'}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-neutral-500">Recommended Distance:</p>
                      <p className="text-sm text-neutral-700">
                        {selectedRequest.originalDistance 
                          ? `${selectedRequest.originalDistance} km`
                          : 'N/A'}
                      </p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-700">
                        {selectedRequest.attachmentPath ? (
                          <a href={selectedRequest.attachmentPath} target="_blank" rel="noopener noreferrer" className="text-primary-500 hover:text-primary-600">
                            View Attachment
                          </a>
                        ) : selectedRequest.attachmentRequired ? (
                          <span className="text-amber-600">Required - Not uploaded</span>
                        ) : (
                          'Not required'
                        )}
                      </p>
