...
export class DatabaseStorage implements IStorage {
  ...
  async getProjectSpending(projectId: number): Promise<{ totalAllocated: number; totalSpent: number; availableBudget: number; budgetUtilization: number; originalBudget: number; effectiveBudget: number }> {
    ...
    // Calculate budget allocations from budget history (for approved but unpaid trips)
    ...
    const grossAllocated = (allocationResult?.totalAllocated || 0);
    const totalDeallocated = (allocationResult?.totalDeallocated || 0);

    // Calculate actual spending from trip_requests table (paid trips only)
    const [spendingResult] = await db
      ...
      .where(
        and(
          ...
          eq(tripRequests.paid, true)
        )
      );

    const totalSpent = spendingResult?.totalSpent || 0;

    // Calculate approved but unpaid trips directly from trip_requests
    const [approvedUnpaidResult] = await db
      .select({
        approvedUnpaid: sql<number>`COALESCE(SUM(${tripRequests.cost}), 0)`
      })
      .from(tripRequests)
      .where(
        and(
    ...
    const approvedUnpaid = approvedUnpaidResult?.approvedUnpaid || 0;
    ...
    // CORRECTED LOGIC: Use actual trip data, not budget history that may be inconsistent
    // Available budget = Total budget - Actually spent - Approved but unpaid
    const availableBudget = effectiveBudget - totalSpent - approvedUnpaid;
    const budgetUtilization = effectiveBudget > 0 ? (totalSpent / effectiveBudget) * 100 : 0;

    return {
      totalAllocated: approvedUnpaid, // Currently approved but not paid
      totalSpent,
      availableBudget,
      budgetUtilization,
      originalBudget,
  ...
  async updateTripRequestStatus(
    ...
    if (approve) {
      ...
      // Budget deduction happens ONLY when FIRST Project Manager step is approved (stepOrder = 1)
      // ALL OTHER APPROVAL TYPES MUST NOT TRIGGER BUDGET DEDUCTION
      if (currentStep.stepType === 'Project Manager' && currentStep.stepOrder === 1 && request.projectId && request.cost) {
        // Handle budget deduction within transaction for atomicity - ONLY for first PM approval
        ...
        if (project) {
          ...
          console.log(`[Budget Deduction] Budget deducted ${request.cost} JD from project ${request.projectId} on FIRST PM approval (step order ${currentStep.stepOrder})`);
        }
      } else if (currentStep.stepType === 'Project Manager' && currentStep.stepOrder > 1) {
        console.log(`[Budget Protection] Skipping budget deduction for secondary PM approval (step order ${currentStep.stepOrder}) - budget already deducted`);
      } else if (currentStep.stepType === 'Department Manager') {
        console.log(`[Budget Protection] Department Manager approval - NO budget deduction (step order ${currentStep.stepOrder})`);
      } else if (currentStep.stepType === 'Second Department Manager') {
        console.log(`[Budget Protection] Second Department Manager approval - NO budget deduction (step order ${currentStep.stepOrder})`);
      } else if (currentStep.stepType === 'Third Department Manager') {
        console.log(`[Budget Protection] Third Department Manager approval - NO budget deduction (step order ${currentStep.stepOrder})`);
      } else if (currentStep.stepType === 'Second Project Manager') {
        console.log(`[Budget Protection] Second Project Manager approval - NO budget deduction (step order ${currentStep.stepOrder})`);
      } else if (currentStep.stepType === 'Finance Approval') {
        console.log(`[Budget Protection] Finance approval - NO budget deduction (step order ${currentStep.stepOrder})`);
      } else {
        console.log(`[Budget Protection] ${currentStep.stepType} approval - NO budget deduction (step order ${currentStep.stepOrder})`);
      }
    } else {
      // Handle rejection - CRITICAL FIX: Update workflow step for rejection
      // Find the current pending workflow step for the rejecting user
      const workflowStepsData = await tx
        .select()
        .from(workflowSteps)
        .where(eq(workflowSteps.tripRequestId, requestId));
      ...
      if (currentStep) {
        // Update the current workflow step to rejected within transaction
        await tx
          .update(workflowSteps)
          .set({
      ...
      updates.rejectionReason = reason || '';
        ...
        // Restore budget ONLY if rejecting after FIRST Project Manager approved (stepOrder = 1)
        // This prevents double restoration from multiple PM rejections
        if (request.projectId && request.cost) {
          // Check if FIRST project manager (stepOrder = 1) has already approved
          const firstProjectManagerStep = await tx
            .select()
            .from(workflowSteps)
            .where(
              and(
                ...
                eq(workflowSteps.stepOrder, 1), // CRITICAL: Only check first PM step
                eq(workflowSteps.status, 'Approved' as any)
              )
            );
          
          // If FIRST project manager approved (stepOrder=1), budget was deducted and needs restoration
          if (firstProjectManagerStep.length > 0) {
            const [project] = await tx
              .select()
              .from(projects)
              .where(eq(projects.id, request.projectId));
            ...
            if (project) {
              ...
              console.log(`[Budget Restoration] Budget restored ${request.cost} JD to project ${request.projectId} after rejection (first PM had approved)`);
            }
          } else {
            console.log(`[Budget Protection] No budget restoration needed - first PM never approved (stepOrder=1)`);
          }
        }
      }
  ...
  // Home trip calculation helper
  async calculateHomeTripDistance(userId: number, originSiteName: string, destinationSiteName: string): Promise<{
    distance: number;
    isHomeTrip: boolean;
    homeDeductionKm: number;
    finalDistance: number;
    userHomeGps: string | null;
    isHomeTripOrigin: boolean;
    isHomeTripDestination: boolean;
  }> {
    console.log(`[Home Trip Calc] Starting calculation for user ${userId}: origin="${originSiteName}", destination="${destinationSiteName}"`);
    
    // Get user's home location
    const user = await this.getUser(userId);
    console.log(`[Home Trip Calc] User home location: ${user?.homeLocation}`);
    
    if (!user?.homeLocation) {
      // If no home location, treat as regular trip
      return {
        distance: 0,
        isHomeTrip: false,
        homeDeductionKm: 0,
        finalDistance: 0,
        userHomeGps: null,
        isHomeTripOrigin: false,
        isHomeTripDestination: false
      };
    }

    // Check if this is a home trip
    const isHomeTripOrigin = originSiteName === "HOME";
    const isHomeTripDestination = destinationSiteName === "HOME";
    const isHomeTrip = isHomeTripOrigin || isHomeTripDestination;
    ...
    // Get system settings for home deduction
    const deductionEnabledSetting = await this.getSystemSetting('home_deduction_enabled');
    const deductionKmSetting = await this.getSystemSetting('home_deduction_km');
    const minDistanceSetting = await this.getSystemSetting('home_deduction_min_distance');

    if (deductionEnabledSetting?.settingValue !== 'true') {
      return {
        distance: 0,
        isHomeTrip: true,
        homeDeductionKm: 0,
        finalDistance: 0,
        userHomeGps: user.homeLocation,
        isHomeTripOrigin,
        isHomeTripDestination
      };
    }

    const deductionKm = parseFloat(deductionKmSetting?.settingValue || '0');
    const minDistance = parseFloat(minDistanceSetting?.settingValue || '0');

    // Parse user's home GPS coordinates
    const [userLat, userLng] = user.homeLocation.split(',').map(coord => parseFloat(coord.trim()));

    // Get the other site (non-HOME site)
    const otherSiteName = isHomeTripOrigin ? destinationSiteName : originSiteName;
    console.log(`[Home Trip Calc] Looking up site: "${otherSiteName}"`);
    
    const otherSite = await db.select().from(sites).where(eq(sites.abbreviation, otherSiteName)).limit(1);
    console.log(`[Home Trip Calc] Site lookup result:`, otherSite.length > 0 ? otherSite[0] : 'Not found');

    if (otherSite.length === 0) {
      // Try alternate lookup by english name in case frontend sends full names
      const alternateResult = await db.select().from(sites).where(eq(sites.englishName, otherSiteName)).limit(1);
      console.log(`[Home Trip Calc] Alternate lookup by english name:`, alternateResult.length > 0 ? alternateResult[0] : 'Not found');
      
      if (alternateResult.length === 0) {
        throw new Error(`Site not found: ${otherSiteName}`);
      }
      otherSite.push(alternateResult[0]);
    }

    // Calculate distance from user's home to the other site using API
    let distance = 0;
    
    try {
      // Use OpenRouteService API for real distance calculation (same as regular sites)
      const apiKey = process.env.OPENROUTESERVICE_API_KEY;
      if (!apiKey) {
        throw new Error('OpenRouteService API key not configured');
      }
      ...
      // Prepare coordinates for OpenRouteService (longitude, latitude format)
      const coordinates = [
        [userLng, userLat], // User's home location
        [Number(otherSite[0].gpsLng), Number(otherSite[0].gpsLat)] // Other site
      ];

      console.log(`[Home Trip API] Calculating from user home to ${otherSite[0].englishName}`);
      console.log(`[Home Trip API] Coordinates: [${coordinates[0]}] to [${coordinates[1]}]`);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`[Home Trip API] OpenRouteService API error: ${response.status} - ${errorText}`);
        throw new Error(`OpenRouteService API error: ${response.status}`);
      }

      const data = await response.json();
      console.log('[Home Trip API] OpenRouteService response received');
      
      if (data.routes && data.routes.length > 0) {
        const route = data.routes[0];
        distance = Math.round((route.summary.distance / 1000) * 100) / 100; // Convert meters to km
        console.log(`[Home Trip API] Calculated distance: ${distance} km`);
      } else {
        throw new Error('No route found');
      }

    } catch (error) {
      console.error('[Home Trip API] API error, falling back to straight-line calculation:', error);
      
      // Fallback to straight-line distance calculation if API fails (same as regular sites)
      const straightLineDistance = this.calculateStraightLineDistance(
        userLat, userLng,
        Number(otherSite[0].gpsLat), Number(otherSite[0].gpsLng)
      );

      // Estimate driving distance as 1.4x straight line distance
      distance = Math.round(straightLineDistance * 1.4 * 100) / 100;
      console.log(`[Home Trip API] Fallback distance: ${distance} km`);
    }

    // Apply deduction if trip meets minimum distance requirement
    const shouldApplyDeduction = distance >= minDistance;
    const homeDeductionKm = shouldApplyDeduction ? deductionKm : 0;
    const finalDistance = Math.max(0, distance - homeDeductionKm);
  ...
  async updateTripRequestStatusAtomic(
    ...
    if (approve) {
      ...
      // CRITICAL: Budget deduction unified to prevent double deductions
      // Budget deduction happens ONLY when FIRST Project Manager step is approved (stepOrder = 1)
      // This function uses allocation system instead of direct deduction to maintain audit trail
      if (currentStep.stepType === 'Project Manager' && currentStep.stepOrder === 1 && request.projectId && request.cost) {
        // Create budget allocation entry - ONLY for first PM approval 
        await dbContext
          .insert(projectBudgetHistory)
          .values({
        ...
        console.log(`[Budget Allocation] Budget allocated ${request.cost} JD from project ${request.projectId} on FIRST PM approval (step order ${currentStep.stepOrder})`);
      } else if (currentStep.stepType === 'Project Manager' && currentStep.stepOrder > 1) {
        console.log(`[Budget Protection] Skipping budget allocation for secondary PM approval (step order ${currentStep.stepOrder}) - budget already allocated`);
      } else if (currentStep.stepType === 'Finance Approval') {
        console.log(`[Budget Protection] Finance approval - NO budget allocation (step order ${currentStep.stepOrder})`);
      } else {
        console.log(`[Budget Protection] ${currentStep.stepType} approval - NO budget allocation (step order ${currentStep.stepOrder})`);
      }
    } else {
      // Handle rejection - CRITICAL FIX: Update workflow step for rejection
      // Find the current pending workflow step for the rejecting user
